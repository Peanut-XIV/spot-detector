...
Jsonable: TypeAlias = dict | list | int | float | str
...
class TreeItem:
    ...
    @classmethod
    def load(
        cls, value: Jsonable, parent: Self | None = None, sort=True
    ) -> "TreeItem":
        ...
        if isinstance(value, dict):
            ...
            for key, value in items:
                child = cls.load(value, rootItem)  # pyright: ignore[reportArgumentType]
                child.key = key
...
class JsonModel(QAbstractItemModel):
    ...
    def flags(self, index: QModelIndex) -> Qt.ItemFlags:  # pyright: ignore[reportIncompatibleMethodOverride, reportAttributeAccessIssue]
        """Override from QAbstractItemModel
