...
from spot_detector.labeling import label_image_fastest_uint16


# --- Alias de types --------------------------------------------------------

RGBImage = NDArray[np.uint16]    # H x W x 3, 16 bits / canal
HueTable = NDArray[np.uint16]    # N x 4 : [r, g, b, category]
BoolMask = NDArray[np.bool_]     # H x W
FloatImage = NDArray[np.float64]  # H x W
...
class BlurResult:
    ...
    channel: int          # canal retenu (0=R, 1=G, 2=B)
...
def select_signal_channel(hue_table: HueTable) -> int:
    """Retourne le canal (0=R, 1=G, 2=B) qui separe le mieux les teintes de
    ...
    dans le fond."""
    rgb = hue_table[:, :3].astype(np.float64)
    category = hue_table[:, 3]
    is_signal = category != 0
    if not np.any(is_signal):
        raise ValueError("La table de teintes ne contient aucune teinte de signal.")
    signal_mean = rgb[is_signal].mean(axis=0)
    if np.any(~is_signal):
...
def get_roi_mask(img: RGBImage, roi: BoolMask | None) -> BoolMask:
...
def compute_signal_mask(img: RGBImage, hue_table: HueTable, roi_mask: BoolMask) -> BoolMask:
    ...
    labels = label_image_fastest_uint16(img, palette)  # H x W, index de teinte
    is_signal_hue = hue_table[:, 3] != 0               # N, bool
...
def dilate_mask(mask: BoolMask, radius_px: int) -> BoolMask:
...
def estimate_density(signal_mask: BoolMask, roi_mask: BoolMask) -> tuple[int, float]:
...
def contrast_normalized_channel(
    ...
    ch = img[:, :, channel].astype(np.float64)
...
def sharpness_score(norm_channel: FloatImage, eval_mask: BoolMask) -> float:
