...
class ImageView:
    ...
    def move_left(self, n: float = 1.0) -> None:
        if self.x > 0:
            self.x -= int(self.scaled_width * 0.1 * n)
        self.x = max(self.x, 0)

    def move_right(self, n: float = 1.0) -> None:
    ...
    def move_up(self, n: float = 1.0) -> None:
        if self.y > 0:
            self.y -= int(self.scaled_height * 0.1 * n)
        self.y = max(self.y, 0)

    def move_down(self, n: float = 1.0) -> None:
...
def reshape_image(img: NDArray, img_view: ImageView) -> NDArray:
    if img_view.scale == 1:
        return img
    crop: NDArray = img[
        img_view.y : img_view.y + img_view.scaled_height,
        img_view.x : img_view.x + img_view.scaled_width,
    ]
    try:
        reshaped = cv.resize(
            crop, None, None, img_view.scale, img_view.scale, cv.INTER_NEAREST
        )
...
def run_gui(labeled_img: NDArray, palette: NDArray) -> list[int]:
    ...
    while index >= 0:
        ...
        shown_img = draw_palette(shown_img, palette, index, color_categories)
        cv.imshow("palette tool", shown_img)
        key = cv.waitKey()
        image_view.window_action(key)
        index, color_categories = other_actions(
            key, index, palette_size, color_categories
        )
...
def draw_all_highlights(labeled_img: NDArray, palette: NDArray):
    ...
    for i in range(palette.shape[0]):
        ...
        hl_img = hl_img.reshape((labeled_img.shape[0], labeled_img.shape[1], 3))
